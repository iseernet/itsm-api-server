...
export class ServiceFaultTypeFinalService {
    static async getAll(payload: ServiceFaultTypeQuery) {
        const pageIndex = payload.pageIndex || 1;
        const pageSize = payload.pageSize || 10;
        const offset = (pageIndex - 1) * pageSize;

        let baseQuery = 'SELECT * FROM service_fault_type_final';
        let countQuery = 'SELECT COUNT(*) FROM service_fault_type_final WHERE parent_id IS NULL';
        const params: any[] = [];
        ...
        if (conditions.length > 0) {
            ...
            countQuery =
                'SELECT COUNT(*) FROM service_fault_type_final WHERE parent_id IS NULL' +
        ...
        const [countResult, rootResult, allResult] = await Promise.all([
            ...
            pool.query('SELECT * FROM service_fault_type_final') // 查全部，方便组树
        ]);
...
export const getFinalServiceFaultTypeByCode = async (code: string): Promise<ServiceFaultTypeQuery[]> => {
    const query = `
        SELECT *
        FROM service_fault_type_final
        WHERE code=$1
    `;
...
export const getAllFinalServiceFaultType = async (): Promise<ServiceFaultType[]> => {
    const query = `
        SELECT *
        FROM service_fault_type_final
...
export const getGroupedFinalServiceFaultTypes = async (): Promise<Record<string, Record<string, ServiceFaultType>>> => {
    const records = await getAllFinalServiceFaultType();
